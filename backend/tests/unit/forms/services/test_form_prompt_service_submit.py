...
class TestFormPromptServiceSubmit:
    ...
    @pytest.mark.asyncio
    async def test_submit_emits_telemetry_with_signal_latency(self) -> None:
        """Submission includes signal delivery latency in telemetry."""
        prompt_id = uuid4()
        prompt = Mock(spec=FormPrompt)
        prompt.id = prompt_id
        prompt.status = FormPromptStatus.PENDING
        ...
        with (
            ...
            # Should include signal delivery latency
            event = mock_dispatcher.dispatch.call_args.args[0]
            assert event.signal_delivery_latency_ms is not None
            assert event.signal_delivery_latency_ms >= 0
    ...
    async def test_submit_signal_failure_still_commits(self) -> None:
        ...
        with (
            ...
            # Should still emit telemetry (with null signal latency)
            event = mock_dispatcher.dispatch.call_args.args[0]
            assert event.signal_delivery_latency_ms is None
