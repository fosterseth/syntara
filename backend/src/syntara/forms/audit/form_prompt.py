...
class FormPromptSubmittedEvent:
    ...
    signal_delivery_latency_ms: int | None = field(default=None)  # ms to deliver signal to workflow
...
class FormPromptSubmittedHandler(AuditEventHandler[FormPromptSubmittedEvent]):
    ...
    def handle(self, event: FormPromptSubmittedEvent) -> AuditEvent:
        ...
        # Add signal delivery latency if available
        if event.signal_delivery_latency_ms is not None:
            data.signal_delivery_latency_ms = event.signal_delivery_latency_ms
